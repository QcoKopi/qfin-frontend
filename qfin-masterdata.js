...
function onMasterDataJenisChange() {
    const mdJenis = document.getElementById('mdJenis');
    if (!mdJenis) return;  // FIX: null check

    const jenis = mdJenis.value;
...
async function submitMasterDataForm() {
    ...
    if (jenis === 'Master_Resep') {
        const namaResep = document.getElementById('mdResepNama');
        const idProdukJadi = document.getElementById('mdResepProdukJadi');
        if (!namaResep) return;

        if (!namaResep.value) { showAlert('danger', 'Nama Resep tidak boleh kosong', 'masterDataAlert'); return; }
        const bahanRows = document.getElementById('mdResepBahanRows');
        if (!bahanRows || bahanRows.children.length === 0) { showAlert('danger', 'Tambahkan minimal 1 bahan baku', 'masterDataAlert'); return; }

        let successCount = 0, failMessages = [];
        for (const row of Array.from(bahanRows.children)) {
            const id = row.id.replace('resep-bahan-row-', '');
            const bahanInput = document.getElementById(`resep-bahan-${id}`);
            if (!bahanInput || !bahanInput.value) continue;
            const result = await callGAS('submitMasterData', {
                table: 'Master_Resep',
                nama_resep: namaResep.value,
                id_produk_jadi: idProdukJadi ? (idProdukJadi.dataset.value || '') : '',
                bahan: bahanInput.value,
            ...
            else failMessages.push(bahanInput.value + ': ' + result.message);
        }
        if (successCount > 0) {
            ...
            if (namaResep) namaResep.value = '';
            if (idProdukJadi) { idProdukJadi.value = ''; idProdukJadi.dataset.value = ''; }
            if (bahanRows) bahanRows.innerHTML = '';
            addResepBahanRow();
    ...
    if (result.success) {
        ...
        const mdResepProdukJadi = document.getElementById('mdResepProdukJadi');
        if (mdResepProdukJadi) mdResepProdukJadi.dataset.value = '';
        localStorage.removeItem(CACHE_KEY + jenis);
        await loadMasterData();
        loadMasterDataList();
    } else {
...
async function loadMasterDataList() {
    ...
    const jenis = mdJenis.value;
    const mdListTableBody = document.getElementById('mdListTableBody');
    if (mdListTableBody) mdListTableBody.innerHTML = '<tr><td>Memuat...</td></tr>';
...
function renderMasterDataList() {
    ...
    const jenis = mdJenis.value;
    const nameField = MD_NAME_FIELD[jenis];
    const mdListSearch = document.getElementById('mdListSearch');
    const query = (mdListSearch ? mdListSearch.value : '').toLowerCase();
